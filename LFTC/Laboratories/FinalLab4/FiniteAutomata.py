...
class FiniteAutomata:
    ...
    first line should contain all states separated by ,
    ...
    def verifySeq(self, seq):
        ...
        while len(seq) != 0:
            if self.finalState(currentState):
                if currentState not in dict.keys():
                    break
            ok = 0
            value = seq[0]
            if value not in self.E:
                return 0
            for element in dict[currentState]:
                for key, v in element.items():
                    if key == value:
                        ...
                        break
            if ok == 0:
                if not self.finalState(currentState):
                    seq = seq[1:]
                else:
                    return 0

        if len(seq) == 0 and self.finalState(currentState):
            return 1
        return 0
    ...
    def menu(self):
        ...
        print("Press 6 to see if it is deterministic\n")
